...
class CommandStaffRoster:

    def __init__(self, bot, soldiers):
        self.bot = bot
        self.staff = []
    ...
    def __getattr__(self, attribute_name):
        ...
        for soldier in self.staff:
            if hasattr(soldier, attribute_name):
                _out = getattr(soldier, attribute_name)
                return _out
        ...
        for soldier in self.staff:
            if hasattr(soldier, attribute_name):
                if iscoroutinefunction(getattr(soldier, attribute_name)):
                    await getattr(soldier, attribute_name)(*args, **kwargs)
                else:
                    getattr(soldier, attribute_name)(*args, **kwargs)

    def retire_troops(self):
        for soldier in self.staff:
            soldier.retire()
    ...
    def __repr__(self) -> str:
        return f"{str(self)}({', '.join([str(soldier) for soldier in self.staff])})"
