...
from antipetros_discordbot.utility.misc import color_hex_embed
...
class ErrorHandler(CommandStaffSoldierBase):
    ...
    async def handle_errors(self, ctx, error):
        await self.error_handle_table.get(type(error), self._default_handle_error)(ctx, error)

    async def _default_handle_error(self, ctx, error):
        log.error('Ignoring exception in command {}:'.format(ctx.command))
        log.error(str(error), exc_info=True)
        await self.bot.message_creator(embed=await self.error_reply_embed(ctx, error, 'Unset Error occured', msg=str(error)))
    ...
    async def _handle_command_on_cooldown(self, ctx, error):
        await ctx.channel.send(embed=await self.error_reply_embed(ctx, error, 'STOP SPAMMING!', f'{ctx.author.mention}\n{ZERO_WIDTH}\n **Your mother was a hamster and your father smelt of elderberries!**'), delete_after=60)
        await ctx.message.delete()
        log.info("Error '%s' was caused by '%s' on the command '%s' with args '%s'", error.__class__.__name__, ctx.author.name, ctx.command.name, ctx.args)

    async def _handle_max_concurrency(self, ctx, error):
        await ctx.channel.send(embed=await self.error_reply_embed(ctx, error, 'STOP SPAMMING!', f'{ctx.author.mention}\n{ZERO_WIDTH}\n **Your mother was a hamster and your father smelt of elderberries!**'), delete_after=60)
        await ctx.message.delete()
        log.info("Error '%s' was caused by '%s' on the command '%s' with args '%s'", error.__class__.__name__, ctx.author.name, ctx.command.name, ctx.args)

    async def error_reply_embed(self, ctx, error, title, msg):
